...
double solveNVDROM(int I, int S, double *uz, double sigma, double *p, double **z, double *c, double d, int Moos, double **xis_oos, double *prob, double *uu, double *prob_valid) {
	...
	while (flag == 0 && iter < Iter &&avg_time1_iter < 7200) {
		...
		for (i = 0; i < I; i++) {
			for (s = 0; s < S; s++) {
				rmatind[nzcnt] = t[i][s];
				rmatval[nzcnt++] = -1.* prob[s] * sigma;
			}
		}
		for (i = 0; i < I; i++) {
			for (s = 0; s < S; s++) {
				rmatind[nzcnt] = rho[i][s];
				rmatval[nzcnt++] = -1. *prob[s] * z[i][s];
			}
		}
		for (i = 0; i < I; i++) {
			for (s = 0; s < S; s++) {
				rmatind[nzcnt] = rho1[i][s];
				rmatval[nzcnt++] = prob[s] * z[i][s];
			}
		}
