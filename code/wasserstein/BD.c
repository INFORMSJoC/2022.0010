...
int r1, I, S;
double theta, d;
int tau1, tau2, *tau3, *tau4, *tau5, *tau6, **tau7, **tau8, *tau9, *tau10, **tau11, **tau12;
int iter, Iter;
double UB, LB;
double lambda;
...
int main(int  argc, char *argv[]) {
	...
	*/
	...
	for (r1 = 0; r1 < num_inst; r1++) {
		...
		//if (fscanf(ini, "%lf", &d) == 1) confirm++;
		if (confirm < 5)  goto TERMINATE;
		out = Open_File(outfile, "a+");
		fprintf(out, "%d;\t %d\t %d\t %s\t%s\t %.4lf\n", r1 + 1, I, S, instance, inst_name, theta);
		fclose(out);

		solution = Open_File(xvalue, "a+");
		fprintf(solution, "%d;\t %d\t %d\t %s\t%s\t %.4lf\n", r1 + 1, I, S, instance, inst_name, theta);
		fclose(solution);
		...
		tau3 = create_int_vector(I);
		tau4 = create_int_vector(I);
		tau5 = create_int_vector(I);
		tau6 = create_int_vector(I);
		tau7 = create_int_matrix(I, I);
		tau8 = create_int_matrix(I, I);
		tau9 = create_int_vector(I);
		tau10 = create_int_vector(I);
		tau11 = create_int_matrix(I, I);
		...
		for (i = 0; i < I; i++) {
			tau3[i] = num_tau2 + num_tau1 + indexd;
			indexd++;
		}
		int num_tau3 = indexd;
		indexd = 0;
		...
		for (s = 0; s < I; s++) {
			tau5[s] = num_tau4 + num_tau3 + num_tau2 + num_tau1 + indexd;
			indexd++;
		}
		int num_tau5 = indexd;
		indexd = 0;
		...
		for (s = 0; s < I; s++) {
			tau6[s] = num_tau5 + num_tau4 + num_tau3 + num_tau2 + num_tau1 + indexd;
			indexd++;
		}
		int num_tau6 = indexd;
		indexd = 0;
		...
		for (s = 0; s < I; s++) {
			tau9[s] = num_tau8 + num_tau7 + num_tau6 + num_tau5 + num_tau4 + num_tau3 + num_tau2 + num_tau1 + indexd;
			indexd++;
		}
		int num_tau9 = indexd;
		indexd = 0;
		...
		for (s = 0; s < I; s++) {
			tau10[s] = num_tau9 + num_tau8 + num_tau7 + num_tau6 + num_tau5 + num_tau4 + num_tau3 + num_tau2 + num_tau1 + indexd;
			indexd++;
		}
		...
		int num_tau = num_tau12 + num_tau11 + num_tau10 + num_tau9 + num_tau8 + num_tau7 + num_tau6 + num_tau5 + num_tau4 + num_tau3 + num_tau2 + num_tau1 + indexd;
		Iter = 100;
		iter = 0;
		tau = create_double_tmatrix(Iter, S, num_tau);
		
		int flag_solve = 0;
		while (avg_time1_iter < 7200 && iter < Iter) {
			CPXENVptr env = NULL;
			CPXLPptr  lp = NULL;
			...
			for (s = 0; s < S; s++) {
				alpha[s] = num_x_var + num_t_var + index1;
				index1++;
				double lb = -100000.;
				double ub = CPX_INFBOUND;
				double cost = 1.;
			...
			for (s = 0; s < S; s++) {
				...
				//	printf("iter is %d\t", iter);
					rhs = 0.;
					for (i = 0; i < I; i++) {
						rhs += tau[k][s][tau11[i][i]];
						rhs -= tau[k][s][tau12[i][i]];
						rhs += tau[k][s][tau9[i]] * z[i][s];
					}
					...
					for (i = 0; i < I; i++) {
						rmatind[nzcnt] = x[i];
						rmatval[nzcnt++] = tau[k][s][tau9[i]];
					}
					rmatind[nzcnt] = t;
					rmatval[nzcnt++] = -1.*tau[k][s][tau2];
					for (i = 0; i < I; i++) {
						if (ABS(rhs) > 0.000001 || ABS(tau[k][s][tau9[i]]) > 0.000001 || ABS(tau[k][s][tau2]) > 0.000001) {
							status = CPXaddrows(env, lp, 0, 1, nzcnt, &rhs, &sense,
								&rmatbeg, rmatind, rmatval, NULL, NULL);
							if (status) {
								fprintf(stderr, "Error in CPXaddrows, status = %d.\n", status);
			...
			if (worker_lp_sol_stat == CPX_STAT_OPTIMAL) {
				...
				if (ABS((UB - LB) / UB) <= 0.00001 || UB < LB) {
					flag_solve = 1;
				}
				printf("flag is %d\t", flag_solve);
				if (flag_solve == 0) {
		...
		if (num_r < 10) {
			...
			if (num_r == 9) {
				solution = Open_File(outfile, "a+");
				fprintf(solution, "\n");
				fprintf(solution, "%.4lf\t%.4lf\t%.1lf\n", avg_obj / 10., avg_time / 10., avg_iter / 10.);
				fclose(solution);
...
int compute_subproblem(double *obj, double *x1, int s) {
	...
	for (j = 0; j < I; j++) {
		y21[j][s] = num_y1_var + num_y0_var + num_r_var + index1;
		index1++;
		lb = -CPX_INFBOUND;
		ub = CPX_INFBOUND;
		status = CPXnewcols(env, lp, 1, NULL, &lb, &ub, NULL, NULL);
	...
	w1[s] = num_eta6_var + num_eta5_var + num_eta4_var + num_eta3_var + num_eta2_var + num_eta1_var + num_y2_var + num_y1_var + num_y0_var + num_r_var + index1;
	index1++;
	lb = 0.;
	ub = CPX_INFBOUND;
	status = CPXnewcols(env, lp, 1, NULL, &lb, &ub, NULL, NULL);
	...
	for (i = 0; i < I; i++) {
		w2[i][s] = num_w1_var + num_eta6_var + num_eta5_var + num_eta4_var + num_eta3_var + num_eta2_var + num_eta1_var + num_y2_var + num_y1_var + num_y0_var + num_r_var + index1;
		index1++;
		lb = 0.;
		ub = CPX_INFBOUND;
		status = CPXnewcols(env, lp, 1, NULL, &lb, &ub, NULL, NULL);
	...
	for (i = 0; i < I; i++) {
		w3[i][s] = num_w2_var + num_w1_var + num_eta6_var + num_eta5_var + num_eta4_var + num_eta3_var + num_eta2_var + num_eta1_var + num_y2_var + num_y1_var + num_y0_var + num_r_var + index1;
		index1++;
		lb = 0.;
		ub = CPX_INFBOUND;
		status = CPXnewcols(env, lp, 1, NULL, &lb, &ub, NULL, NULL);
	...
	rmatind = (int *)malloc((I * 3 + 1 + I * I) * sizeof(int));
	if (rmatind == NULL) {
		fprintf(stderr, "No memory for rmatind array.\n");
		status = -1;
		goto TERMINATE;
	}
	rmatval = (double *)malloc((I * 3 + 1 + I * I) * sizeof(double));
	if (rmatval == NULL) {
		fprintf(stderr, "No memory for rmatval array.\n");
		status = -1;
		goto TERMINATE;
...
int solve_projection(double obj, double *x1, double *xk) {
	...
	t = num_x_var + num_alpha_var + index1;
	index1++;
	double lb = 0.;
	double ub = CPX_INFBOUND;
	double cost = theta;
	...
	int rho;
	rho = num_t_var + num_x_var + num_alpha_var + index1;
	index1++;
	lb = 0.;
	ub = CPX_INFBOUND;
	cost = 1.;
	...
	int *quadcol;
	double *quadval;
	quadrow = create_int_vector(I + 1);
	quadcol = create_int_vector(I + 1);
	quadval = create_double_vector(I + 1);
	rmatind = create_int_vector(I + 2);
	rmatval = create_double_vector(I + 2);
	rhs = 0;
	for (i = 0; i < I + 1; i++) {
		rhs -= xk[i] * xk[i];
	}
	...
	rmatind = create_int_vector(I + S+1);
	rmatval = create_double_vector(I + S + 1);
