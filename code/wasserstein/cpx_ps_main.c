...
int main(int  argc, char *argv[]) {
	...
	*/
	...
	for (r = 0; r < num_inst; r++) {
		...
		out = Open_File(outfile, "a+");
		fprintf(out, "%d;\t %d\t %d\t %s\t%s\t %.4lf\n", r + 1, I, S, instance, inst_name, theta);
		fclose(out);
		...
		double obj_out;
		double *obj_iter;
		obj_iter = create_double_vector(Iter);
		double n = 0.;
		while (iter < Iter) {
			obj_out = 0.;
			...
			if (worker_lp_sol_stat == CPX_STAT_OPTIMAL) {
				...
				sparse = 0;
				for (i = 0; i < num_sub; i++) {
					if (ABS(x2[i]) < 0.0000001) {
						sparse++;
					}
				}
				sparse_ = sparse * 1. / num_sub;
				solution = Open_File(xvalue, "a+");
				...
				avg_obj_iter = z1;
				avg_time1_iter += cputimeMIP1;
				solution = Open_File(outfile, "a+");
				fprintf(solution, "%.4lf\t %.4lf\t %.4lf\n", z1, sparse_, avg_time1_iter);
				fclose(solution);
				...
				solution = Open_File(outfile, "a+");
				fprintf(solution, "%.4lf\t%.4lf\t%.2lf\n", best_upper_bound, best_lower_bound, ABS(best_upper_bound - best_lower_bound) / best_upper_bound * 100);
				fclose(solution);
				avg_time1_iter = 7300.;
			}
		...
		if (num_r < 50) {
			...
			if (num_r == 49) {
				solution = Open_File(outfile, "a+");
				fprintf(solution, "\n");
				fprintf(solution, "%.4lf\t %.4lf\t %.1lf\t %.2lf\n", avg_obj / fea_num, avg_time1 / fea_num, fea_num / 10., avg_sparse / fea_num * 100);
				fclose(solution);
