...
int main(int  argc, char *argv[]) {
	...
	int        status = 0;
	char       inst_name[40];
	strcpy(inst_name, "input_500.txt");
	/*
	if (argc == 1) {
	...
	*/
	...
	for (rk = 0; rk < num_inst; rk++) {
		...
		out = Open_File(outfile, "a+");
		fprintf(out, "%d;\t %d\t %d\t %s\t%s\t%.1lf\n", rk + 1, I, S, instance, inst_name, sigma);
		fclose(out);
		...
		cstat = create_int_vector(1000);
		rstat = create_int_vector(1000);
		while (flag == 0 && iter < Iter &&avg_time1_iter<7200) {
			...
			worker_lp_sol_stat = CGsolver(iter, d, c, p, uz, z, I, S, sigma, Ly1, Ly11, Leta1, Leta2, Leta3, Leta4, Leta5, Leta6, env, lp, &z1, x2, tau);
			end_time1 = clock();
			...
			if (worker_lp_sol_stat == CPX_STAT_OPTIMAL) {
				if (iter == 0) {
					...
					cstat = create_int_vector(num_sub);
					rstat = create_int_vector(num_tau);
				}
				...
				else if (status == CPX_STAT_INFEASIBLE) {
					printf("Model is infeasible\n");
					goto TERMINATE;
				}
				else if (status == CPX_STAT_INForUNBD) {
				...
				int i2, i3, i4, i5, i6, j3, j4, j5, j6;
				min_eta1 = 1000.;
				min_eta2 = 1000.;
				double fmin_eta3, fmin_eta4, fmin_eta5, fmin_eta6;
				fmin_eta3 = fmin_eta4 = fmin_eta5 = fmin_eta6 = 1000.;
				...
				for (i = 0; i < I; i++) {
					...
					if (min_eta3 < fmin_eta3) {
						fmin_eta3 = min_eta3;
					}
					if (min_eta4 < fmin_eta4) {
						fmin_eta4 = min_eta4;
					}
					if (min_eta5 < fmin_eta5) {
						fmin_eta5 = min_eta5;
					}
					if (min_eta6 < fmin_eta6) {
						fmin_eta6 = min_eta6;
					}
					if (min_eta3 < -0.000001) {
						for (s = 0; s < S; s++) {
							ql = (linklist*)malloc(sizeof(linklist));
							ql->data = i * I*S + j3 * S + s;
							ple3->next = ql;                   //  利用尾差法  
							ple3 = ql;
							ple3->next = NULL;
							avg_col_iter++;
					...
					if (min_eta4 < -0.000001) {
						for (s = 0; s < S; s++) {
							ql = (linklist*)malloc(sizeof(linklist));
							ql->data = i * I*S + j4 * S + s;
							ple4->next = ql;                   //  利用尾差法  
							ple4 = ql;
							ple4->next = NULL;
							avg_col_iter++;
					...
					if (min_eta5 < -0.000001) {
						for (s = 0; s < S; s++) {
							ql = (linklist*)malloc(sizeof(linklist));
							ql->data = i * I*S + j5 * S + s;
							ple5->next = ql;                   //  利用尾差法  
							ple5 = ql;
							ple5->next = NULL;
							avg_col_iter++;
					...
					if (min_eta6 < -0.000001) {
						for (s = 0; s < S; s++) {
							ql = (linklist*)malloc(sizeof(linklist));
							ql->data = i * I*S + j6 * S + s;
							ple6->next = ql;                   //  利用尾差法  
							ple6 = ql;
							ple6->next = NULL;
							avg_col_iter++;
				...
				solution = Open_File(outfile, "a+");
				fprintf(solution, "%.4lf\t%.4lf\t%.4lf\t%.4lf\t%d\n", avg_obj_iter, avg_time_iter, avg_time1_iter, avg_time2_iter, avg_col_iter);
				fclose(solution);
